...
def _high_shelf_coefficients(sample_rate):
    """K-weighting pre-filter coefficients for any supported sample rate."""
...
def _high_pass_coefficients(sample_rate):
    """K-weighting RLB high-pass filter coefficients."""
    frequency, quality = 38.135, 0.5
    omega = 2.0 * np.pi * frequency / sample_rate
    sine, cosine = np.sin(omega), np.cos(omega)
    alpha = sine / (2.0 * quality)
    b = np.array([(1.0 + cosine) / 2.0, -(1.0 + cosine), (1.0 + cosine) / 2.0])
    a = np.array([1.0 + alpha, -2.0 * cosine, 1.0 - alpha])
    return b / a[0], a / a[0]


def energy_to_lufs(energy):
    """Convert K-weighted mean-square energy to LUFS."""
    if energy <= 0.0:
        return SILENCE_LUFS
    return max(SILENCE_LUFS, LUFS_OFFSET + 10.0 * np.log10(energy))
...
class LoudnessMeter:
    ...
    def reset(self, sample_rate=None, channels=None):
        ...
        high_pass_b, high_pass_a = _high_pass_coefficients(self.sample_rate)
        self._filters = (
    ...
    def process(self, samples):
        ...
        for b, a, state in self._filters:
            weighted, new_state = lfilter(b, a, weighted, axis=0, zi=state)
            updated_filters.append((b, a, new_state))
        self._filters = tuple(updated_filters)

        self._momentary_samples = self._append_window(
            self._momentary_samples, weighted, 0.4
        )
        self._short_term_samples = self._append_window(
            self._short_term_samples, weighted, 3.0
        )
        self._block_samples = np.vstack((self._block_samples, weighted))
    ...
    def _append_window(self, existing, new_samples, seconds):
        ...
        return np.vstack((existing, new_samples))[-maximum:]

    @staticmethod
    def _energy(samples):
        if len(samples) == 0:
            return 0.0
        return float(np.sum(np.mean(np.square(samples), axis=0)))
    ...
    def _store_complete_blocks(self):
        ...
        while len(self._block_samples) >= block_size:
            ...
            self._block_energies.append(self._energy(block))
            self._block_samples = self._block_samples[block_size:]
    ...
    def _integrated_lufs(self):
        ...
        blocks = np.convolve(energies, np.ones(4) / 4.0, mode="valid")
        block_lufs = np.array([energy_to_lufs(value) for value in blocks])
        absolute_gated = blocks[block_lufs > SILENCE_LUFS]
        if len(absolute_gated) == 0:
            return SILENCE_LUFS

        relative_gate = energy_to_lufs(float(np.mean(absolute_gated))) - 10.0
        gated_lufs = np.array([energy_to_lufs(value) for value in absolute_gated])
        gated = absolute_gated[gated_lufs > relative_gate]
        return energy_to_lufs(float(np.mean(gated))) if len(gated) else SILENCE_LUFS